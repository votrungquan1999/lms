...
test.describe("Redo Flow", () => {
  ...
  test("setup: student authenticates and saves auth state", async ({
    ...
    await context.storageState({
      path: path.join(authDir, "redo-student.json"),
    });
  ...
  test("setup: student answers and submits the test", async ({ browser }) => {
    ...
    await page
      ...
      .fill("A function that calls itself.");
  ...
  test("admin sees a 'Request Redo' button for the student on the grading page", async ({
    page,
  }) => {
    await page.goto("/admin/courses");
    await page.getByText(COURSE_TITLE).click();
    ...
    await expect(
      page.getByRole("button", { name: "Request Redo" }),
    ).toBeVisible();
  });

  // ─── Scenario 2: Admin clicks Request Redo ────────────────────────────────

  test("admin can request a redo — button is replaced by confirmation text", async ({
    page,
  }) => {
    await page.goto("/admin/courses");
    await page.getByText(COURSE_TITLE).click();
    ...
    await expect(page.getByText("Redo requested")).toBeVisible({
      timeout: 10000,
    });
    await expect(
      page.getByRole("button", { name: "Request Redo" }),
  ...
  test("student sees a 'Redo Required' banner after admin requested a redo", async ({
    browser,
  }) => {
    const context = await browser.newContext({
      storageState: path.join(authDir, "redo-student.json"),
    ...
    await expect(page.getByText("Redo Required")).toBeVisible({
      timeout: 10000,
    });
    await expect(
      page.getByText("Your teacher has requested that you redo this test"),
    ).toBeVisible();
  ...
  test("grading page still shows 'Redo requested' after a page reload", async ({
    page,
  }) => {
    await page.goto("/admin/courses");
    await page.getByText(COURSE_TITLE).click();
  ...
  test("student can re-answer and resubmit the test after a redo request", async ({
    browser,
  }) => {
    const context = await browser.newContext({
      storageState: path.join(authDir, "redo-student.json"),
    ...
    await page
      .getByPlaceholder("Type your answer here...")
      .fill("Recursion is when a function calls itself with a base case.");
    await page.getByRole("button", { name: "Submit Answer" }).click();
    await expect(page.getByRole("button", { name: "Edit Answer" })).toBeVisible(
      { timeout: 10000 },
    );
